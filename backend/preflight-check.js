  ...
  if (hasDbHost && hasDbName && hasDbUser && hasDbPassword) {
    ...
    if (envContent.includes("DB_PASSWORD=postgres")) {
      checks.warnings.push(
        "Using default password. Update DB_PASSWORD in .env"
      );
...
exec("psql --version", (error, stdout, stderr) => {
  if (error) {
    checks.warnings.push(
      "PostgreSQL client (psql) not found. Install PostgreSQL first."
    );
    console.log(
      "   ⚠️  PostgreSQL client not found. Is PostgreSQL installed?\n"
    );
  } else {
  ...
  sequelize
    ...
    .then(([results]) => {
      if (results.length === 0) {
        checks.warnings.push(
          "No tables found. Run seed script after first start."
        );
        console.log(
          "   ⚠️  No tables found yet. Will be created on first run.\n"
        );
      } else {
    ...
    .catch((err) => {
      ...
      console.log(
        '      - Create database: psql postgres -c "CREATE DATABASE client_management;"'
      );
...
function printSummary() {
  ...
  if (checks.failed.length === 0) {
    ...
    console.log(
      "   4. Login at http://localhost:3000 with admin@example.com / admin123"
    );
    console.log("");
  } else {
    console.log(
